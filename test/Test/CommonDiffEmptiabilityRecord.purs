...
import Test.Common (class Common, SkipBroken(..), assertSkipHelper, makeCollection, printTestType)
...
testDeRec ::
  forall a c cInverse canEmpty nonEmpty cPattern.
  Eq (canEmpty Int) =>
  Eq (c (nonEmpty Int)) ⇒
  SkipBroken ->
  c Int ->
  ...
  Effect Unit
testDeRec skip _ nil _ { makeCollection
, toCanEmpty
, toNonEmpty
, catMaybes
, drop
...
, stripPrefix
  ...
  let
    ...
    cel ∷ ∀ b. Array b → canEmpty b
    cel x = toCanEmpty (makeCollection x)

    nel ∷ ∀ b. Array b → nonEmpty b
  ...
  assert $ (drop 1 (makeCollection [ 1, 2, 3 ])) == (cel [ 2, 3 ])
  ...
  assertSkip [ SkipBrokenLazyCanEmpty ] \_ -> groupAll (l [ 1, 2, 2, 3, 3, 3, 1 ]) == l [ nel [ 1, 1 ], nel [ 2, 2 ], nel [ 3, 3, 3 ] ]
  ...
  assertSkip [ SkipBrokenLazyCanEmpty ] \_ -> (takeEnd 1 (l [ 1, 2, 3 ])) == cel [ 3 ]
  assertSkip [ SkipBrokenLazyCanEmpty ] \_ -> (takeEnd 2 (l [ 1, 2, 3 ])) == cel [ 2, 3 ]
  assertSkip [ SkipBrokenLazyCanEmpty ] \_ -> (takeEnd 2 (l [ 1 ])) == cel [ 1 ]
